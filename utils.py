...
class EduPages(Charts):
    ...
    def make_summary_change(self, df):
        ...
        # ---------------------------------------------------------------------------------------------------------------
        # 신청누계 및 수료누계 병한
        df_summary = pd.merge(pd.DataFrame({'월': sorted(df_summary_attend.index, key=self.sort_month)}), 
                          pd.concat([df_summary_apply, df_summary_attend], axis=0), on=['월'])
        return df_summary   
