...
class MakeSet(CallData):
    ...
    def make_set_change(self, df, *columns): # *columns : '소속부문' 또는 '입사연차'
        ...
        for i in range(len(self.index)):
            ...
            df_two_unique = df[df[df.columns[self.index[i][0]]] != 0].groupby([*columns])['사원번호'].count().reset_index(name=self.index[i[1]]) # 수료현황 또는 IMO신청여부 : 값이 1인 사원번호 개수 구하기 (수료인원 및 IMO신청인원)
