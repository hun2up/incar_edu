...
class ServiceData:
    ...
    def make_set_branch(self, df):
        ...
        for channel in df_branch['소속부문'].unique():
            ...
            df_part_result = pd.DataFrame() # 소속부문별 산하 소속총괄의 월별 접속자수 합계 (소계 포함)
            for part in df_channel['소속총괄'].unique():
                ...
                if part == ['전략1총괄']: st.dataframe(df_part_result)
                else: pass
        # ---------------------------------------------------------------------------------------------------------------
            df_sum = pd.DataFrame() # 산하 소속총괄의 월별 접속자수 합계
            for channel_detail in df_channel.columns:
                if channel_detail in ['소속부문']: df_sum[channel_detail] = df_channel[channel_detail]
                elif channel_detail in ['소속총괄','소속부서']: df_sum[channel_detail] = ''
                else: df_sum[channel_detail] = df_channel[channel_detail].sum()
            df_sum = df_sum.iloc[[0]] # 소속부서 개수만큼 생성된 동일한 내용의 ROW를 하나만 남기고 삭제
            df_total = pd.concat([df_total, df_sum], axis=0)
        return df_total
