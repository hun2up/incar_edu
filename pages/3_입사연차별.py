...
if authentication_status:
    ...
    df_career = instance.call_data_pages()
    st.header("입사연차별 교육지표") # 페이지 타이틀

    # --------------------------------------------          사이드바          ------------------------------------------------
    # 사이드바 헤더
    st.sidebar.header("원하는 옵션을 선택하세요")
    #사이드바 제작
    year = make_sidebar(df_career,'년도')
    ...
    # 데이터와 사이드바 연결
    df_career = df_career.query("년도 == @year & 월 == @month & 지역 == @region & 보험사 == @partner & 과정형태 == @line & 과정분류 == @theme & 과정명 == @name & 소속부문 == @channel & 입사연차 == @career")
    ...
    line_attend.plotly_chart(instance.make_linechart(
        ...
        xaxis='월',
        yaxis='수료누계',
        title='입사연차별 수료인원 추이 (수료누계 기준)'), use_container_width=True)
    ...
    line_imo_rate.plotly_chart(instance.make_linechart(
        ...
        xaxis='월',
        yaxis='IMO신청률',
        title='입사연차별 IMO신청률 추이'), use_container_width=True)
