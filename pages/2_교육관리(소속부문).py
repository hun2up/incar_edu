...
if authentication_status:
    ...
    # ------------------------------------------          인스턴스 생성          ---------------------------------------------
    instance = Chart()
    df_channel = instance.call_data_change("attend", '소속부문')
    st.dataframe(instance.make_set_status(df_channel,'소속부문',*['소속부문']))
    #instance.make_set_change(df_channel,*['소속부문'])
    ...
    hbar_attend_people.plotly_chart(instance.make_hbarchart_group(
        ...
        axis_a='재적인원 대비 수료인원',
        axis_b='재적인원 대비 수료누계',
        title='소속부문별 재적인원 대비 교육수료 현황'), use_container_width=True)
